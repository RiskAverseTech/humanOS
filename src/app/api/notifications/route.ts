...
import { getActivityFeed, type ActivityFeedItem } from '@/lib/activity/feed'
import type { NotificationCategory } from '@/lib/supabase/types'

type NotificationItem = ActivityFeedItem & {
  category: NotificationCategory
  unread: boolean
}

const CATEGORY_MAP: Record<ActivityFeedItem['type'], NotificationCategory> = {
  note: 'notes',
  document: 'vault',
  todo: 'todos',
...
export async function GET() {
  ...
  const rawItems = enabled ? await getActivityFeed({ perSourceLimit: 20, maxItems: 40 }) : []
