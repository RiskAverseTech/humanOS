...
export default async function ActivityPage() {
  ...
  const [
    ...
    supabase.from('human_chat_channels').select('id, name, updated_at, owner_id').order('updated_at', { ascending: false }).limit(50),
  ...
  for (const x of humanChats ?? []) items.push({ type: 'human_chat', title: x.name || 'Human Chat', href: `/channels/${x.id}`, date: x.updated_at, icon: '🗨️', ownerName: ownerNames[x.owner_id] })
